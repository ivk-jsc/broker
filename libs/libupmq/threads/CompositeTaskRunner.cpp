...
class CompositeTaskRunnerImpl {
 ...
 public:
  CompositeTaskRunnerImpl() : tasks(), mutex(), thread(), threadTerminated(false), pending(false), shutdown(false) {}
};
}  // namespace threads
}  // namespace upmq
...
bool CompositeTaskRunner::isStarted() const {
  ...
  synchronized(&impl->mutex) {
    if (this->impl->thread != nullptr && this->impl->thread->isAlive()) {
      result = true;
    }
  }
