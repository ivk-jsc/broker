...
class Pointer : public REFCOUNTER {
 ...
 public:
  ...
  using CounterType = REFCOUNTER;  // Type of the Reference Counter
 ...
 public:
   ...
   */
  void reset(T *newValue = nullptr) { Pointer(newValue).swap(*this); }
   ...
   */
  PointerType get() const { return this->value; }
   ...
   */
  void swap(Pointer &newValue) noexcept {
    std::swap(this->value, newValue.value);
    REFCOUNTER::swap(newValue);
  }

  void swap(Pointer &&newValue) noexcept {
    this->value = newValue.release();
    REFCOUNTER::swap(std::forward<REFCOUNTER>(newValue));
  }
  ...
  /**
   * Assigns the rvalue of right to this Pointer and not increments the reference Count.
   * @param right - Pointer on the right hand side of an operator= call to this.
   */
  Pointer &operator=(Pointer &&right) noexcept {
    if (this == &right) {
      return *this;
    }
    Pointer temp(std::move(right));
    temp.swap(*this);
    return *this;
  }
  ...
  ReferenceType operator*() {
    if (this->value == nullptr) {
      throw decaf::lang::exceptions::NullPointerException(__FILE__, __LINE__, "Pointer operator& - Pointee is NULL.");
    }
  ...
  bool operator!() const { return this->value == nullptr; }

  friend bool operator==(const Pointer &left, const T *right) { return left.get() == right; }

  friend bool operator==(const T *left, const Pointer &right) { return left == right.get(); }
...
class PointerComparator : public decaf::util::Comparator<Pointer<T, R> > {
 public:
  PointerComparator() = default;
  PointerComparator(const PointerComparator &) = default;
  PointerComparator(PointerComparator &&) = default;
  PointerComparator &operator=(const PointerComparator &) = default;
  PointerComparator &operator=(PointerComparator &&) = default;
  ~PointerComparator() override = default;

  // Allows for operator less on types that implement Comparable or provide
  // a workable operator <
  bool operator()(const Pointer<T, R> &left, const Pointer<T, R> &right) const override { return *left < *right; }

  // Requires that the type in the pointer is an instance of a Comparable.
  int compare(const Pointer<T, R> &left, const Pointer<T, R> &right) const override { return *left < *right ? -1 : *right < *left ? 1 : 0; }
};
}  // namespace lang
}  // namespace decaf
