...
////////////////////////////////////////////////////////////////////////////////
SocketImplFactory *Socket::factory = nullptr;

////////////////////////////////////////////////////////////////////////////////
Socket::Socket() : impl(nullptr), created(false), connected(false), closed(false), bound(false), inputShutdown(false), outputShutdown(false) {
...
Socket::Socket(const InetAddress *address, int port)
  ...
  try {
    ...
    this->initSocketImpl(address->getHostAddress(), port, nullptr, 0);
  }
...
Socket::Socket(const std::string &host, int port)
  ...
  try {
    ...
    this->initSocketImpl(host, port, nullptr, 0);
  }
  DECAF_CATCH_RETHROW(UnknownHostException)
  DECAF_CATCH_RETHROW(IOException)
  DECAF_CATCH_EXCEPTION_CONVERT(Exception, IOException)
...
////////////////////////////////////////////////////////////////////////////////
int Socket::getPort() const { return this->connected ? this->impl->getPort() : 0; }

////////////////////////////////////////////////////////////////////////////////
int Socket::getLocalPort() const { return this->bound ? this->impl->getLocalPort() : -1; }

////////////////////////////////////////////////////////////////////////////////
std::string Socket::getInetAddress() const { return this->connected ? this->impl->getInetAddress() : ""; }

////////////////////////////////////////////////////////////////////////////////
std::string Socket::getLocalAddress() const { return this->bound ? this->impl->getLocalAddress() : "0.0.0.0"; }
...
void Socket::connect(const std::string &host, int port, int timeout) {
  ...
  if (timeout < 0) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Timeout value specified is invalid: %d", timeout);
  }

  if (isConnected()) {
    throw SocketException(__FILE__, __LINE__, "The Socket is already connected.");
  }

  if (host.empty()) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Host cannot be empty.");
  }
...
void Socket::setSoLinger(bool state, int timeout) {
  ...
  try {
    ...
    if (state && timeout < 0) {
      throw IllegalArgumentException(__FILE__, __LINE__, "Timeout value passed is invalid: %d", timeout);
    }
...
void Socket::setReceiveBufferSize(int size) {
  ...
  if (size <= 0) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Buffer size given was invalid: %d", size);
  }
...
void Socket::setSoTimeout(int timeout) {
  ...
  if (timeout < 0) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Socket timeout given was invalid: %d", timeout);
  }
...
void Socket::setTrafficClass(int value) {
  ...
  if (value < 0 || value > 255) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Value of the Traffic class option was invalid: %d", value);
  }
...
////////////////////////////////////////////////////////////////////////////////
void Socket::setSocketImplFactory(SocketImplFactory *newFactory) {
  if (Socket::factory != nullptr) {
    throw SocketException(__FILE__, __LINE__, "A SocketInplFactory was already set.");
  }

  Socket::factory = newFactory;
}
