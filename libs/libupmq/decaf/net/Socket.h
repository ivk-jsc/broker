...
class DECAF_API Socket : public decaf::io::Closeable {
 ...
 public:
   ...
   */
  bool isInputShutdown() const { return inputShutdown; }

  /**
   * @return true if output on this Socket has been shutdown.
   */
  bool isOutputShutdown() const { return outputShutdown; }
   ...
   *
   * @return the local address to which the socket is bound or InetAddress.anyLocalAddress() if the socket is not
   * bound yet.
   */
 ...
 public:
   ...
   */
  static void setSocketImplFactory(SocketImplFactory *newFactory);
