...
class DECAF_API SocketImpl : public SocketOptions {
 ...
 public:
   ...
   */
  virtual void connect(const std::string &hostname, int bindPort, int timeout) = 0;
   ...
   * @throws IOException if an I/O error occurs while attempting this operation.
   */
  virtual void bind(const std::string &ipaddress, int bindPort) = 0;
   ...
   */
  int getLocalPort() const { return this->localPort; }
   ...
   */
  std::string getInetAddress() const { return this->address; }
   ...
   */
  const decaf::io::FileDescriptor *getFileDescriptor() const { return this->fd; }
   ...
   */
  virtual bool supportsUrgentData() const { return false; }
