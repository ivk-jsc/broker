...
////////////////////////////////////////////////////////////////////////////////
void DataOutputStream::doWriteArrayBounded(const unsigned char *pbuffer, int size, int offset, int length) {
  if (length == 0) {
    return;
  }
  ...
  try {
    outputStream->write(pbuffer, size, offset, length);
    written += length;
  }
  DECAF_CATCH_RETHROW(IOException)
...
void DataOutputStream::writeInt(int value) {
  try {
    ...
    *reinterpret_cast<int *>(buffer) = value;
...
void DataOutputStream::writeLong(long long value) {
  try {
    ...
    *reinterpret_cast<long long *>(buffer) = value;
...
void DataOutputStream::writeUTF(const std::string &value) {
  try {
    size_t utfLength = this->countUTFLength(value);

    if (utfLength > 65535) {
      throw UTFDataFormatException(__FILE__,
                                   __LINE__,
    ...
    std::vector<unsigned char> utfBytes(utfLength);
    unsigned int utfIndex = 0;

    for (std::size_t i = 0; i < length; i++) {
      unsigned int charValue = value.at(i);
      ...
      if (charValue > 0 && charValue <= 127) {
        utfBytes[utfIndex++] = static_cast<unsigned char>(charValue);
      } else if (charValue <= 2047) {
        utfBytes[utfIndex++] = static_cast<unsigned char>(0xc0 | (0x1f & (charValue >> 6)));
        utfBytes[utfIndex++] = static_cast<unsigned char>(0x80 | (0x3f & charValue));
      } else {
        utfBytes[utfIndex++] = static_cast<unsigned char>(0xe0 | (0x0f & (charValue >> 12)));
        utfBytes[utfIndex++] = static_cast<unsigned char>(0x80 | (0x3f & (charValue >> 6)));
        utfBytes[utfIndex++] = static_cast<unsigned char>(0x80 | (0x3f & charValue));
      }
    }

    this->writeUnsignedShort(static_cast<unsigned short>(utfLength));
    if (utfLength > 0) {
