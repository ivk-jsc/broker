...
////////////////////////////////////////////////////////////////////////////////
BufferedOutputStream::BufferedOutputStream(OutputStream *stream, bool own) : FilterOutputStream(stream, own), buffer(nullptr), bufferSize(0), head(0), tail(0) {
  // Default to 1k buffer.
  init(8192);
}

////////////////////////////////////////////////////////////////////////////////
BufferedOutputStream::BufferedOutputStream(OutputStream *stream, int bufSize, bool own) : FilterOutputStream(stream, own), buffer(nullptr), bufferSize(0), head(0), tail(0) {
  try {
    this->init(bufSize);
  }
...
////////////////////////////////////////////////////////////////////////////////
void BufferedOutputStream::doWriteArray(const unsigned char *pbuffer, int size) {
  try {
    if (size == 0) {
      return;
    }
...
////////////////////////////////////////////////////////////////////////////////
void BufferedOutputStream::doWriteArrayBounded(const unsigned char *pbuffer, int size, int offset, int length) {
  try {
    if (length == 0) {
      return;
    }
    ...
    for (int pos = 0; pos < length;) {
      ...
      System::arraycopy(pbuffer, offset + pos, this->buffer, this->tail, bytesToWrite);
