...
class DECAF_API DataInputStream : public FilterInputStream {
 ...
 public:
   ...
   */
  virtual void readFully(unsigned char *pbuffer, int size);
   ...
   */
  virtual void readFully(unsigned char *pbuffer, int size, int offset, int length);
 ...
 private:
  ...
  void readAllData(unsigned char *pbuffer, int length);
};
}  // namespace io
}  // namespace decaf
