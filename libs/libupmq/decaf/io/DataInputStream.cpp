...
std::string DataInputStream::readString() {
  try {
    ...
    int size = 1024;
    std::vector<char> symBuffer;
    symBuffer.resize(size);
    int pos = 0;

    while (true) {
      if (inputStream->read((unsigned char *)(&symBuffer[0]), size, pos, 1) == -1) {
...
std::string DataInputStream::readUTF() {
  try {
    ...
    std::vector<unsigned char> symBuffer(utfLength);
    std::vector<unsigned char> result(utfLength);

    this->readFully(&symBuffer[0], utfLength);
    ...
    while (count < utfLength) {
      if ((result[index] = symBuffer[count++]) < 0x80) {
        index++;
        ...
        unsigned char b = symBuffer[count++];
        if ((b & 0xC0) != 0x80) {
...
////////////////////////////////////////////////////////////////////////////////
void DataInputStream::readFully(unsigned char *pbuffer, int size) {
  try {
    if (size == 0) {
      return;
    }

    this->readFully(pbuffer, size, 0, size);
  }
...
////////////////////////////////////////////////////////////////////////////////
void DataInputStream::readFully(unsigned char *pbuffer, int size, int offset, int length) {
  try {
    if (length == 0) {
      return;
    }
    ...
    int n = 0;
    while (n < length) {
      int count = inputStream->read(pbuffer, length, offset + n, length - n);
      if (count == -1) {
...
////////////////////////////////////////////////////////////////////////////////
void DataInputStream::readAllData(unsigned char *pbuffer, int length) {
  try {
    int n = 0;
    do {
      int count = inputStream->read(pbuffer, length, n, length - n);
      if (count == -1) {
