...
////////////////////////////////////////////////////////////////////////////////
PushbackInputStream::PushbackInputStream(InputStream *stream, bool own_)
    : FilterInputStream(stream, own_), buffer(new unsigned char[1]), bufferSize(1), pos(1) {}

////////////////////////////////////////////////////////////////////////////////
PushbackInputStream::PushbackInputStream(InputStream *stream, int bufSize, bool own_)
    : FilterInputStream(stream, own_), buffer(nullptr), bufferSize(bufSize), pos(bufSize) {
  if (bufSize < 0) {
...
////////////////////////////////////////////////////////////////////////////////
void PushbackInputStream::mark(int readLimit DECAF_UNUSED) { DECAF_UNUSED_VAR(readLimit); }

////////////////////////////////////////////////////////////////////////////////
void PushbackInputStream::reset() { throw IOException(__FILE__, __LINE__, "Reset is not Supported by the PushbackInputStream."); }
...
////////////////////////////////////////////////////////////////////////////////
void PushbackInputStream::unread(const unsigned char *pbuffer, int size) {
  if (size < 0) {
    throw IndexOutOfBoundsException(__FILE__, __LINE__, "Given size of the buffer was negatiev.");
  }

  try {
    this->unread(pbuffer, size, 0, size);
  }
  DECAF_CATCH_RETHROW(IOException)
...
////////////////////////////////////////////////////////////////////////////////
void PushbackInputStream::unread(const unsigned char *pbuffer, int size, int offset, int length) {
  if (length > pos) {
    throw IOException(__FILE__, __LINE__, "No Space left in the unread buffer.");
  }
  ...
  try {
    System::arraycopy(pbuffer, offset, this->buffer, pos - length, length);
    pos = pos - length;
  }
  DECAF_CATCH_RETHROW(IOException)
...
////////////////////////////////////////////////////////////////////////////////
int PushbackInputStream::doReadArrayBounded(unsigned char *pbuffer, int size, int offset, int length) {
  if (isClosed()) {
    throw IOException(__FILE__, __LINE__, "Stream is closed");
  }
  ...
  try {
    ...
    if (pos < bufferSize) {
      ...
      System::arraycopy(this->buffer, pos, pbuffer, newOffset, copyLength);
    ...
    int inCopied = inputStream->read(pbuffer, size, newOffset, length - copiedBytes);
    if (inCopied > 0) {
