...
class LatchSync : public AbstractQueuedSynchronizer {
 ...
 public:
  LatchSync(int count) { this->setState(count); }

  ~LatchSync() override = default;

  int getCount() const { return getState(); }

 protected:
  int tryAcquireShared(int acquires DECAF_UNUSED) override {
    DECAF_UNUSED_VAR(acquires);
    return getState() == 0 ? 1 : -1;
  }

  bool tryReleaseShared(int releases DECAF_UNUSED) override {
    DECAF_UNUSED_VAR(releases);
    for (;;) {
...
////////////////////////////////////////////////////////////////////////////////
CountDownLatch::CountDownLatch(int count) : sync(nullptr) {
  if (count < 0) {
...
////////////////////////////////////////////////////////////////////////////////
int CountDownLatch::getCount() const { return this->sync->getCount(); }

////////////////////////////////////////////////////////////////////////////////
std::string CountDownLatch::toString() const { return std::string("CountDownLatch[count = ") + Integer::toString(this->sync->getCount()) + "]"; }
