 ...
 public:  // RejectedExecutionHandler implementations.
  ...
  class AbortPolicy : public RejectedExecutionHandler {
   public:
    ...
    ~AbortPolicy() override = default;

    void rejectedExecution(decaf::lang::Runnable *task, ThreadPoolExecutor *executer DECAF_UNUSED) override {
      DECAF_UNUSED_VAR(executer);
      delete task;
  ...
  class DiscardOldestPolicy : public RejectedExecutionHandler {
   public:
    DiscardOldestPolicy() = default;

    void rejectedExecution(decaf::lang::Runnable *task, ThreadPoolExecutor *executer) override {
      if (executer->isShutdown()) {
        delete task;
        return;
      }
      ...
      try {
        decaf::lang::Runnable *oldest = nullptr;
