...
class DefaultThreadFactory : public ThreadFactory {
 ...
 public:
  DefaultThreadFactory() : ThreadFactory(), threadNumber(1), namePrefix() {
    if (DefaultThreadFactory::poolNumber == nullptr) {
      throw NullPointerException();
    }

    namePrefix = std::string("pool-") + Integer::toString(poolNumber->getAndIncrement()) + "-thread-";
  }
...
class NonConfigurableExecutorService : public AbstractExecutorService {
 ...
 public:
  ...
  virtual void execute(decaf::lang::Runnable *command) { this->service->execute(command); }

  virtual void execute(decaf::lang::Runnable *command, bool takeOwnership) { this->service->execute(command, takeOwnership); }

  virtual bool awaitTermination(long long timeout, const TimeUnit &unit) { return this->service->awaitTermination(timeout, unit); }

  virtual void shutdown() { this->service->shutdown(); }

  virtual ArrayList<decaf::lang::Runnable *> shutdownNow() { return this->service->shutdownNow(); }

  virtual bool isShutdown() const { return this->service->isShutdown(); }

  virtual bool isTerminated() const { return this->service->isTerminated(); }
};

AtomicInteger *DefaultThreadFactory::poolNumber = nullptr;
}  // namespace
...
////////////////////////////////////////////////////////////////////////////////
void Executors::initialize() { DefaultThreadFactory::poolNumber = new AtomicInteger(1); }

////////////////////////////////////////////////////////////////////////////////
void Executors::shutdown() { delete DefaultThreadFactory::poolNumber; }

////////////////////////////////////////////////////////////////////////////////
ThreadFactory *Executors::getDefaultThreadFactory() { return new DefaultThreadFactory(); }
...
ExecutorService *Executors::newFixedThreadPool(int nThreads) {
  ...
  try {
    ...
    ExecutorService *service = new ThreadPoolExecutor(nThreads, nThreads, 0, TimeUnit::MILLISECONDS, backingQ.get());
...
ExecutorService *Executors::newFixedThreadPool(int nThreads, ThreadFactory *threadFactory) {
  ...
  try {
    backingQ.reset(new LinkedBlockingQueue<Runnable *>());
    ExecutorService *service = new ThreadPoolExecutor(nThreads, nThreads, 0, TimeUnit::MILLISECONDS, backingQ.get(), threadFactory);
...
ExecutorService *Executors::newSingleThreadExecutor() {
  ...
  try {
    backingQ.reset(new LinkedBlockingQueue<Runnable *>());
    ExecutorService *service = new ThreadPoolExecutor(1, 1, 0, TimeUnit::MILLISECONDS, backingQ.get());
...
ExecutorService *Executors::newSingleThreadExecutor(ThreadFactory *threadFactory) {
  ...
  try {
    backingQ.reset(new LinkedBlockingQueue<Runnable *>());
    ExecutorService *service = new ThreadPoolExecutor(1, 1, 0, TimeUnit::MILLISECONDS, backingQ.get(), threadFactory);
