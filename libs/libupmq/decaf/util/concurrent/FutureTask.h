...
class FutureTask : public RunnableFuture<T> {
 private:
  ...
  class FutureTaskAdapter : public decaf::util::concurrent::Callable<T> {
   ...
   public:
    FutureTaskAdapter(decaf::lang::Runnable *task, const T &result, bool owns = true) : decaf::util::concurrent::Callable<T>(), task(task), callable(nullptr), owns(owns), result(result) {}

    FutureTaskAdapter(decaf::util::concurrent::Callable<T> *task, bool owns = true) : decaf::util::concurrent::Callable<T>(), task(nullptr), callable(task), owns(owns), result(T()) {}
  ...
  class FutureTaskSync : public locks::AbstractQueuedSynchronizer {
   ...
   public:
    FutureTaskSync(FutureTask *parent, Callable<T> *callable) : AbstractQueuedSynchronizer(), callable(callable), result(), exception(), parent(parent), runner(nullptr) {}

    virtual ~FutureTaskSync() {}

    bool innerIsCancelled() const { return getState() == CANCELLED; }

    bool innerIsDone() const { return ranOrCancelled(getState()) && this->runner == nullptr; }
   ...
   protected:
     ...
     */
    virtual int tryAcquireShared(int ignore DECAF_UNUSED) { return innerIsDone() ? 1 : -1; }
   ...
   private:
    bool ranOrCancelled(int state) const { return (state & (RAN | CANCELLED)) != 0; }
  };
 ...
 public:
   ...
   */
  FutureTask(Callable<T> *callable, bool takeOwnership = true) : sync(nullptr) {
    if (callable == nullptr) {
      throw decaf::lang::exceptions::NullPointerException(__FILE__, __LINE__, "The Callable pointer passed to the constructor was NULL");
    }
   ...
   */
  FutureTask(decaf::lang::Runnable *runnable, const T &result, bool takeOwnership = true) : sync(nullptr) {
    if (runnable == nullptr) {
      throw decaf::lang::exceptions::NullPointerException(__FILE__, __LINE__, "The Runnable pointer passed to the constructor was NULL");
    }
  ...
  virtual bool isCancelled() const { return this->sync->innerIsCancelled(); }

  virtual bool isDone() const { return this->sync->innerIsDone(); }

  virtual bool cancel(bool mayInterruptIfRunning) { return this->sync->innerCancel(mayInterruptIfRunning); }

  virtual T get() { return this->sync->innerGet(); }

  virtual T get(long long timeout, const TimeUnit &unit) { return this->sync->innerGet(unit.toNanos(timeout)); }

  FutureTask<T> *clone() { return new FutureTask<T>(*this); }
 ...
 public:
   ...
   */
  virtual void set(const T &result) { this->sync->innerSet(result); }
   ...
   */
  virtual void setException(const decaf::lang::Exception &error) { this->sync->innerSetException(error); }

  virtual void run() { this->sync->innerRun(); }
   ...
   */
  virtual bool runAndReset() { return this->sync->innerRunAndReset(); }
