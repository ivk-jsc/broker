...
class LinkedBlockingQueue : public BlockingQueue<E> {
 private:
  ...
  class QueueNode {
   ...
   public:
    ...
    bool isUnlinked() const { return this->unlinked; }

    bool isDequeued() const { return this->dequeued; }
  };
 ...
 public:
  virtual int size() const { return this->count.get(); }
  ...
  virtual int remainingCapacity() const { return this->capacity - this->count.get(); }
  ...
  virtual bool remove(const E &value) {
    ...
    for (Pointer<QueueNode<E> > predicessor = this->head, p = predicessor->next; p != nullptr; predicessor = p, p = p->next) {
      if (value == p->get()) {
  ...
  virtual std::vector<E> toArray() const {
    ...
    for (Pointer<QueueNode<E> > p = this->head->next; p != nullptr; p = p->next) {
  ...
  virtual int drainTo(Collection<E> &sink, int maxElements) {
    if (&sink == this) {
      throw decaf::lang::exceptions::IllegalArgumentException(__FILE__, __LINE__, "Cannot drain this Collection to itself.");
    }
 ...
 private:
  class LinkedIterator : public Iterator<E> {
   ...
   public:
    ...
    virtual bool hasNext() const { return this->current != nullptr; }

    virtual E next() {
      TotalLock lock(this->parent);

      if (this->current == nullptr) {
        throw decaf::util::NoSuchElementException(__FILE__, __LINE__, "Iterator next called with no matching next element.");
      }

      E result = this->currentElement;
      this->last = this->current;
      this->current = this->nextNode(this->current);
      this->currentElement = (this->current == nullptr) ? E() : this->current->get();
    ...
    virtual void remove() {
      if (this->last == nullptr) {
        throw decaf::lang::exceptions::IllegalStateException(__FILE__, __LINE__, "Iterator remove called without having called next().");
      }
      ...
      for (Pointer<QueueNode<E> > trail = this->parent->head, p = trail->next; p != nullptr; trail = p, p = p->next) {
        if (p == node) {
  ...
  class ConstLinkedIterator : public Iterator<E> {
   ...
   public:
    ...
    virtual void remove() { throw lang::exceptions::UnsupportedOperationException(__FILE__, __LINE__, "Cannot write to a const ListIterator."); }
 ...
 public:
  virtual decaf::util::Iterator<E> *iterator() { return new LinkedIterator(this); }

  virtual decaf::util::Iterator<E> *iterator() const { return new ConstLinkedIterator(this); }
