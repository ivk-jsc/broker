...
class DECAF_API Executors {
 ...
 private:
  ...
  class RunnableAdapter : public decaf::util::concurrent::Callable<E> {
   ...
   public:
    RunnableAdapter(decaf::lang::Runnable *task_, bool owns_, const E &result_)
        : decaf::util::concurrent::Callable<E>(), task(task_), owns(owns_), result(result_) {}
 ...
 public:
  ...
  static Callable<E> *callable(decaf::lang::Runnable *task, bool owns = true) {
    if (task == nullptr) {
      throw decaf::lang::exceptions::NullPointerException(__FILE__, __LINE__, "The Runnable task argument cannot be NULL");
    }
