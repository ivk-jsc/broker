...
class UnownedTaskWrapper : public Runnable {
 ...
 public:
  ...
  virtual void run() { this->task->run(); }
};
...
class ExecutorKernel {
 private:
  ...
  class Worker : public AbstractQueuedSynchronizer, public Runnable {
   ...
   public:
    ...
    virtual void lock() { acquire(1); }

    virtual bool tryLock() { return tryAcquire(1); }

    virtual void unlock() { release(1); }

    virtual bool isLocked() { return isHeldExclusively(); }

   protected:
    bool isHeldExclusively() const override { return getState() == 1; }

    bool tryAcquire(int unused DECAF_UNUSED) override {
      DECAF_UNUSED_VAR(unused);
    ...
    bool tryRelease(int unused DECAF_UNUSED) override {
      DECAF_UNUSED_VAR(unused);
      this->setExclusiveOwnerThread(nullptr);
      this->setState(0);
      return true;
    }
  };
 ...
 public:
  ...
  ~ExecutorKernel() {
    try {
      ...
      // run recently.
      Pointer<Iterator<Worker *> > tpWorkers(this->deadWorkers.iterator());
      while (tpWorkers->hasNext()) {
        Worker *worker = tpWorkers->next();
        worker->thread->join();
        delete worker;
      }
  ...
  bool tryTerminate() {
    for (;;) {
      ...
      {
        ScopedLock scopedLock(mainLock);
        try {
   ...
   */
  void interruptIdleWorkers() { this->interruptIdleWorkers(false); }
  ...
  void clearInterruptsForTaskRun() {
    if (this->runStateLessThan(ctl.get(), STOP) && Thread::interrupted() && this->runStateAtLeast(ctl.get(), STOP)) {
      ...
      while (task != nullptr || (task = getTask()) != nullptr) {
        ...
        try {
          ...
          try {
            ...
            Exception ex(__FILE__, __LINE__, new std::exception(stdex), "Caught unknown exception while executing task.");
          ...
          this->parent->afterExecute(task, nullptr);
  ...
  bool isShutdown() const { return !isRunning(ctl.get()); }
  ...
  bool isTerminated() const { return runStateAtLeast(ctl.get(), TERMINATED); }
  ...
  void setCorePoolSize(int aCorePoolSize) {
    int delta = aCorePoolSize - this->corePoolSize;

    this->corePoolSize = aCorePoolSize;

    if (workerCountOf(ctl.get()) > aCorePoolSize) {
      interruptIdleWorkers();
    } else if (delta > 0) {
  ...
  bool prestartCoreThread() { return workerCountOf(ctl.get()) < corePoolSize && addWorker(nullptr, true); }
  ...
  void allowCoreThreadTimeOut(bool value) {
    if (value && keepAliveTime <= 0) {
      throw IllegalArgumentException(__FILE__, __LINE__, "Core threads must have nonzero keep alive times");
    }
  ...
  void setKeepAliveTime(long long time, const TimeUnit &unit) {
    ...
    long long aKeepAliveTime = unit.toNanos(time);
    long long delta = aKeepAliveTime - this->keepAliveTime;
  ...
  void purge() {
    ...
    try {
      ...
      while (iter->hasNext()) {
        ...
        if (r != nullptr && future != nullptr && future->isCancelled()) {
          iter->remove();
        }
      }
 ...
 private:
  static bool runStateLessThan(int c, int s) { return c < s; }

  static bool runStateAtLeast(int c, int s) { return c >= s; }

  static bool isRunning(int c) { return c < SHUTDOWN; }
 ...
 private:
  ...
  retry:
    for (;;) {
      ...
      if (rs >= SHUTDOWN && !(rs == SHUTDOWN && firstTask == nullptr && !workQueue->isEmpty())) {
        return false;
      }
  ...
  success:
    ...
    try {
      ...
      if (t == nullptr || (rs >= SHUTDOWN && !(rs == SHUTDOWN && firstTask == NULL))) {
   ...
   */
  bool compareAndIncrementWorkerCount(int expect) { return ctl.compareAndSet(expect, expect + 1); }

  /**
   * Attempt to CAS-decrement the workerCount field of ctl.
   */
  bool compareAndDecrementWorkerCount(int expect) { return ctl.compareAndSet(expect, expect - 1); }
...
// runState is stored in the high-order bits
const int ExecutorKernel::RUNNING = -1 << ExecutorKernel::COUNT_BITS;  //-V610
...
ThreadPoolExecutor::ThreadPoolExecutor(
  ...
  try {
    ...
    this->kernel = new ExecutorKernel(this, corePoolSize, maxPoolSize, unit.toMillis(keepAliveTime), workQueue, threadFactory.get(), handler.get());
...
ThreadPoolExecutor::ThreadPoolExecutor(int corePoolSize,
  ...
  try {
    ...
    this->kernel = new ExecutorKernel(this, corePoolSize, maxPoolSize, unit.toMillis(keepAliveTime), workQueue, threadFactory.get(), handler);
...
ThreadPoolExecutor::ThreadPoolExecutor(int corePoolSize,
  ...
  try {
    ...
    this->kernel = new ExecutorKernel(this, corePoolSize, maxPoolSize, unit.toMillis(keepAliveTime), workQueue, threadFactory, handler.get());
...
ThreadPoolExecutor::ThreadPoolExecutor(int corePoolSize,
  ...
  try {
    ...
    this->kernel = new ExecutorKernel(this, corePoolSize, maxPoolSize, unit.toMillis(keepAliveTime), workQueue, threadFactory, handler);
  }
  DECAF_CATCH_RETHROW(NullPointerException)
  DECAF_CATCH_RETHROW(IllegalArgumentException)
  DECAF_CATCH_RETHROW(Exception)
...
void ThreadPoolExecutor::execute(Runnable *task) {
  try {
    if (task == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "ThreadPoolExecutor::execute - Supplied Runnable pointer was NULL.");
    }
...
////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::getPoolSize() const { return this->kernel->getPoolSize(); }

////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::getCorePoolSize() const { return this->kernel->corePoolSize; }
...
////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::getMaximumPoolSize() const { return this->kernel->maxPoolSize; }
...
////////////////////////////////////////////////////////////////////////////////
long long ThreadPoolExecutor::getTaskCount() const { return this->kernel->getTaskCount(); }

////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::getActiveCount() const { return this->kernel->getActiveCount(); }

////////////////////////////////////////////////////////////////////////////////
long long ThreadPoolExecutor::getCompletedTaskCount() const { return this->kernel->getCompletedTaskCount(); }

////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::getLargestPoolSize() const { return this->kernel->getLargestPoolSize(); }
...
////////////////////////////////////////////////////////////////////////////////
ThreadFactory *ThreadPoolExecutor::getThreadFactory() const { return this->kernel->factory.get(); }

////////////////////////////////////////////////////////////////////////////////
RejectedExecutionHandler *ThreadPoolExecutor::getRejectedExecutionHandler() const { return this->kernel->rejectionHandler.get(); }
...
////////////////////////////////////////////////////////////////////////////////
BlockingQueue<Runnable *> *ThreadPoolExecutor::getQueue() { return this->kernel->workQueue.get(); }

////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::isShutdown() const { return this->kernel->isShutdown(); }

////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::isTerminated() const { return this->kernel->isTerminated(); }

////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::isTerminating() const { return this->kernel->isTerminating(); }
...
////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::allowsCoreThreadTimeout() const { return this->kernel->coreThreadsCanTimeout; }

////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::prestartCoreThread() { return this->kernel->prestartCoreThread(); }

////////////////////////////////////////////////////////////////////////////////
int ThreadPoolExecutor::prestartAllCoreThreads() { return this->kernel->prestartAllCoreThreads(); }

////////////////////////////////////////////////////////////////////////////////
bool ThreadPoolExecutor::remove(decaf::lang::Runnable *task) { return this->kernel->remove(task); }

////////////////////////////////////////////////////////////////////////////////
void ThreadPoolExecutor::purge() { this->kernel->purge(); }

////////////////////////////////////////////////////////////////////////////////
void ThreadPoolExecutor::beforeExecute(Thread *thread DECAF_UNUSED, Runnable *task DECAF_UNUSED) {
  DECAF_UNUSED_VAR(thread);
  DECAF_UNUSED_VAR(task);
}

////////////////////////////////////////////////////////////////////////////////
void ThreadPoolExecutor::afterExecute(Runnable *task DECAF_UNUSED, decaf::lang::Throwable *error DECAF_UNUSED) {
  DECAF_UNUSED_VAR(task);
  DECAF_UNUSED_VAR(error);
}
