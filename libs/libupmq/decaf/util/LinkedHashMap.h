...
class LinkedHashMap : public HashMap<K, V, HASHCODE> {
 ...
 private:
   ...
   public:
    ...
    void checkConcurrentMod() const {
      if (expectedModCount != associatedMap->modCount) {
        throw ConcurrentModificationException(__FILE__, __LINE__, "LinkedHashMap modified outside this iterator");
      }
    }

    virtual bool checkHasNext() const { return (futureEntry != nullptr); }

    void makeNext() {
      checkConcurrentMod();
      if (!checkHasNext()) {
        throw decaf::util::NoSuchElementException(__FILE__, __LINE__, "No next element");
      }
  ...
  class ValueIterator : public Iterator<V>, public AbstractMapIterator {
   ...
   public:
    ...
    virtual void remove() { this->doRemove(); }
  };
 ...
 private:
  ...
  class LinkedHashMapEntrySet : public HashMap<K, V, HASHCODE>::HashMapEntrySet {
   ...
   public:
    LinkedHashMapEntrySet(LinkedHashMap *parent) : HashMap<K, V, HASHCODE>::HashMapEntrySet(parent), associatedMap(parent) {}

    virtual ~LinkedHashMapEntrySet() {}

    virtual Iterator<MapEntry<K, V> > *iterator() { return new EntryIterator(associatedMap); }
  ...
  class ConstLinkedHashMapEntrySet : public HashMap<K, V, HASHCODE>::ConstHashMapEntrySet {
   ...
   public:
    ConstLinkedHashMapEntrySet(const LinkedHashMap *parent) : HashMap<K, V, HASHCODE>::ConstHashMapEntrySet(parent), associatedMap(parent) {}
 ...
 private:
  class LinkedHashMapKeySet : public HashMap<K, V, HASHCODE>::HashMapKeySet {
   ...
   public:
    LinkedHashMapKeySet(LinkedHashMap *parent) : HashMap<K, V, HASHCODE>::HashMapKeySet(parent), associatedMap(parent) {}

    virtual ~LinkedHashMapKeySet() {}

    virtual Iterator<K> *iterator() { return new KeyIterator(this->associatedMap); }

    virtual Iterator<K> *iterator() const { return new ConstKeyIterator(this->associatedMap); }
  };
  ...
  class ConstLinkedHashMapKeySet : public HashMap<K, V, HASHCODE>::ConstHashMapKeySet {
   ...
   public:
    ConstLinkedHashMapKeySet(const LinkedHashMap *parent) : HashMap<K, V, HASHCODE>::ConstHashMapKeySet(parent), associatedMap(parent) {}
 ...
 private:
  class LinkedHashMapValueCollection : public HashMap<K, V, HASHCODE>::HashMapValueCollection {
   ...
   public:
    LinkedHashMapValueCollection(LinkedHashMap *parent) : HashMap<K, V, HASHCODE>::HashMapValueCollection(parent), associatedMap(parent) {}

    virtual ~LinkedHashMapValueCollection() {}

    virtual Iterator<V> *iterator() { return new ValueIterator(this->associatedMap); }

    virtual Iterator<V> *iterator() const { return new ConstValueIterator(this->associatedMap); }
  };
 ...
 public:
   ...
   */
  LinkedHashMap(int capacity, float load) : HashMap<K, V, HASHCODE>(capacity, load), accessOrder(false), head(), tail() {}
   ...
   *     If the capacity is less than zero or the load factor is less or equal to zero.
   */
  LinkedHashMap(int capacity, float load, bool order) : HashMap<K, V, HASHCODE>(capacity, load), accessOrder(order), head(), tail() {}
 ...
 protected:
   ...
   */
  virtual bool removeEldestEntry(const MapEntry<K, V> &eldest DECAF_UNUSED) { return false; }
 ...
 public:
  ...
  virtual std::string toString() const { return "LinkedHashMap"; }

 protected:
  LinkedHashMapEntry *getEntry(const K &key) const override {
    LinkedHashMapEntry *result = nullptr;
    ...
    if (result != nullptr && accessOrder && tail != result) {
  ...
  typename HashMap<K, V, HASHCODE>::HashMapEntry *createEntry(const K &key, int index, const V &value) override {
  ...
  virtual bool putImpl(const K &key, const V &value, V &oldValue) {
    ...
    entry = dynamic_cast<LinkedHashMapEntry *>(this->findKeyEntry(key, index, hash));
    if (entry == nullptr) {
      ++this->modCount;
      ...
      entry = dynamic_cast<LinkedHashMapEntry *>(this->createHashedEntry(key, index, hash));
      replaced = false;
    } else {
