...
class StlSet : public decaf::util::AbstractSet<E> {
   ...
   public:
    ...
    virtual E next() {
      if (this->current == set->end()) {
        throw NoSuchElementException(__FILE__, __LINE__, "Set::Iterator::next - No more elements to return");
      }
    ...
    virtual void remove() {
      if (this->previous == set->end()) {
        throw lang::exceptions::IllegalStateException(__FILE__, __LINE__, "Set::Iterator::remove - Invalid State to call remove");
      }
 ...
 public:
   ...
   */
  StlSet(const StlSet &source) : AbstractCollection<E>(), AbstractSet<E>(), values() { copy(source); }

  /**
   * Copy constructor - copies the content of the given set into this one.
   * @param source The source set.
   */
  StlSet(const Collection<E> &source) : AbstractSet<E>(), values() { AbstractSet<E>::copy(source); }
   ...
   * {@inheritDoc}
   */
  Iterator<E> *iterator() { return new SetIterator(&values); }
  Iterator<E> *iterator() const { return new ConstSetIterator(&values); }
   ...
   */
  virtual int size() const { return (int)values.size(); }

  /**
   * {@inheritDoc}
   */
  virtual bool add(const E &value) { return values.insert(value).second; }

  /**
   * {@inheritDoc}
   */
  virtual bool remove(const E &value) { return values.erase(value) != 0; }
};
}  // namespace util
}  // namespace decaf
