...
enum ParsingMode { PARSE_MODE_NONE = 0, PARSE_MODE_SLASH = 1, PARSE_MODE_CONTINUE = 2, PARSE_MODE_KEY_DONE = 3, PARSE_MODE_IGNORE = 4 };
...
bool Properties::isEmpty() const {
  bool isEmpty = false;
  synchronized(&(internal->properties)) { isEmpty = internal->properties.isEmpty(); }
  return isEmpty;
}
...
int Properties::size() const {
  int size = 0;
  synchronized(&(internal->properties)) { size = static_cast<int>(internal->properties.size()); }
  return size;
}
...
const char *Properties::getProperty(const std::string &name) const {
  synchronized(&(internal->properties)) {
    ...
    } else if (this->defaults != nullptr && this->defaults->hasProperty(name)) {
...
bool Properties::hasProperty(const std::string &name) const {
  ...
  synchronized(&(internal->properties)) { hasProperty = this->internal->properties.containsKey(name); }
...
std::vector<std::pair<std::string, std::string> > Properties::toArray() const {
  ...
  synchronized(&(internal->properties)) {
    Pointer<Iterator<MapEntry<std::string, std::string> > > entries(this->internal->properties.entrySet().iterator());
...
void Properties::copy(const Properties &source) {
  ...
  synchronized(&(this->internal->properties)) {
    synchronized(&(source.internal->properties)) {
      ...
      if (source.defaults.get() != nullptr) {
...
void Properties::clear() {
  synchronized(&(internal->properties)) { this->internal->properties.clear(); }
}

////////////////////////////////////////////////////////////////////////////////
bool Properties::equals(const Properties &source) const { return this->internal->properties.equals(source.internal->properties); }
...
void Properties::load(decaf::io::InputStream *stream) {
  try {
    if (stream == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "The Stream instance passed was Null");
    }
        ...
        switch (nextChar) {
          ...
          case '\r':
            ...
            if (offset > 0 || (offset == 0 && keyLength == 0)) {
              ...
              this->internal->properties.put(temp.substr(0, (std::size_t)keyLength), temp.substr((std::size_t)keyLength));
            }
...
void Properties::load(decaf::io::Reader *reader) {
  try {
    if (reader == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "The Reader instance passed was Null");
    }
...
void Properties::store(decaf::io::OutputStream *out, const std::string &comments DECAF_UNUSED) {
  try {
    if (out == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "The OutputStream instance passed was Null");
    }
...
void Properties::store(decaf::io::Writer *writer, const std::string &comments DECAF_UNUSED) {
  DECAF_UNUSED_VAR(comments);
  try {
    if (writer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "The Writer instance passed was Null");
    }
