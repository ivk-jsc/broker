...
class DECAF_API StringTokenizer {
 ...
 public:
   ...
   * @throws NoSuchElementException if there are no more tokens in this string.
   */
  virtual std::string nextToken(const std::string &aDelim);
   ...
   * @param isReturnDelims
   *      Should the Tokenizer return delimiters as Tokens, default false
   */
  virtual void reset(const std::string &aStr = "", const std::string &aDelim = "", bool isReturnDelims = false);
};
}  // namespace util
}  // namespace decaf
