...
long long TimerTask::scheduledExecutionTime() const {
  ...
  synchronized(&lock) { result = this->scheduledTime; }
...
bool TimerTask::isScheduled() const {
  ...
  synchronized(&lock) { result = when > 0 || scheduledTime > 0; }
...
void TimerTask::setScheduledTime(long long time) {
  synchronized(&lock) { this->scheduledTime = time; }
}
...
long long TimerTask::getWhen() const {
  ...
  synchronized(&lock) { result = this->when; }
