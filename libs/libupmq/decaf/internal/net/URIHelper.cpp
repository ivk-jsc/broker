...
URIType URIHelper::parseURI(const std::string &uri, bool forceServer) {
  ...
  if (index != std::string::npos && (index2 == std::string::npos || index2 >= index) && (index3 == std::string::npos || index3 >= index)) {
    ...
    if (result.getScheme().empty()) {
      throw URISyntaxException(__FILE__, __LINE__, uri, "Scheme not specified.", (int)index);
    }

    validateScheme(uri, result.getScheme(), 0);
    result.setSchemeSpecificPart(temp.substr(index + 1, std::string::npos));

    if (result.getSchemeSpecificPart().empty()) {
      throw URISyntaxException(__FILE__, __LINE__, uri, "Scheme specific part is invalid..", static_cast<int>(index) + 1);
    }
  ...
  if (result.getScheme().empty() || (!result.getSchemeSpecificPart().empty() && result.getSchemeSpecificPart().at(0) == '/')) {
    ...
    if (temp.size() >= 2 && temp.at(0) == '/' && temp.at(1) == '/') {
      ...
      if (index != std::string::npos) {
        ...
        if (result.getAuthority() == "" && result.getQuery() == "" && result.getFragment() == "") {
          throw URISyntaxException(__FILE__, __LINE__, uri, "Scheme specific part is invalid..", (int)uri.length());
        }
      }
...
void URIHelper::validateScheme(const std::string &uri, const std::string &scheme, int index) {
  ...
  if (!Character::isLetter(scheme.at(0))) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Schema must start with a Letter.", (int)index);
  }

  try {
    URIEncoderDecoder::validateSimple(scheme, "+-.");
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid Schema", (int)index + e.getIndex());
  }
}

////////////////////////////////////////////////////////////////////////////////
void URIHelper::validateSsp(const std::string &uri, const std::string &ssp, std::size_t index) {
  try {
    URIEncoderDecoder::validate(ssp, allLegal);
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid URI Ssp", (int)index + e.getIndex());
  }
}

////////////////////////////////////////////////////////////////////////////////
void URIHelper::validateAuthority(const std::string &uri, const std::string &authority, std::size_t index) {
  try {
    // "@[]" + someLegal
    URIEncoderDecoder::validate(authority, "@[]" + someLegal);
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid URI Authority", (int)index + e.getIndex());
  }
}

////////////////////////////////////////////////////////////////////////////////
void URIHelper::validatePath(const std::string &uri, const std::string &path, std::size_t index) {
  try {
    // "/@" + someLegal
    URIEncoderDecoder::validate(path, "/@" + someLegal);
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid URI Path", (int)index + e.getIndex());
  }
}

////////////////////////////////////////////////////////////////////////////////
void URIHelper::validateQuery(const std::string &uri, const std::string &query, std::size_t index) {
  try {
    URIEncoderDecoder::validate(query, allLegal);
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid URI Query", (int)index + e.getIndex());
  }
}

////////////////////////////////////////////////////////////////////////////////
void URIHelper::validateFragment(const std::string &uri, const std::string &fragment, std::size_t index) {
  try {
    URIEncoderDecoder::validate(fragment, allLegal);
  } catch (URISyntaxException &e) {
    throw URISyntaxException(__FILE__, __LINE__, uri, "Invalid URI Fragment", (int)index + e.getIndex());
  }
}
    ...
    if (index != std::string::npos && (endindex < index || endindex == std::string::npos)) {
      ...
      if (index < (temp.length() - 1)) {  // port part is not empty
        try {
          ...
          if (tempPort < 0) {
            if (forceServer) {
              throw URISyntaxException(__FILE__, __LINE__, authority, "Port number is missing", (int)hostindex + (int)index + 1);
            }
          ...
          if (forceServer) {
            throw URISyntaxException(__FILE__, __LINE__, authority, "Port number is malformed.", (int)hostindex + (int)index + 1);
          }
...
////////////////////////////////////////////////////////////////////////////////
void URIHelper::validateUserinfo(const std::string &uri, const std::string &userinfo, std::size_t index) {
  for (std::size_t i = 0; i < userinfo.length(); i++) {
    char ch = userinfo.at(i);
    if (ch == ']' || ch == '[') {
      throw URISyntaxException(__FILE__, __LINE__, uri, "User Info cannot contain '[' or ']'", (int)(index + i));
    }
  }
}
  ...
  try {
    if (host.at(0) == '[') {
      ...
      if (host.at(host.length() - 1) != ']') {
        throw URISyntaxException(__FILE__, __LINE__, host, "Host address does not end in ']'", 0);
      }

      if (!isValidIP6Address(host)) {
        throw URISyntaxException(__FILE__, __LINE__, host, "Host IPv6 address is not valid");
      }
    ...
    if (host.find('[') != std::string::npos || host.find(']') != std::string::npos) {
      throw URISyntaxException(__FILE__, __LINE__, host, "Unexpected '[' or ']' found in address");
    }

    std::size_t index = host.find_last_of('.');

    if (index == std::string::npos || index == host.length() - 1 || !Character::isDigit(host.at(index + 1))) {
      // domain name
      ...
      if (forceServer) {
        throw URISyntaxException(__FILE__, __LINE__, host, "Host address is not valid");
      }
    ...
    if (forceServer) {
      throw URISyntaxException(__FILE__, __LINE__, host, "Host IPv4 address is not valid");
    }
...
bool URIHelper::isValidIP6Address(const std::string &ipAddress) {
  ...
  if (numberOfPeriods > 0) {
    ...
    if (word == "" && ipAddress.at(length - 1 - offset) != ':' && ipAddress.at(length - 2 - offset) != ':') {
      return false;
    }
  }
...
////////////////////////////////////////////////////////////////////////////////
bool URIHelper::isValidHexChar(char c) { return (c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f'); }
