...
void URIEncoderDecoder::validate(const std::string &s, const std::string &legal) {
  ...
  for (std::size_t i = 0; itr != s.end(); ++i, ++itr) {
    ...
    if (ch == '%') {
      if (i + 2 >= s.length()) {
        throw URISyntaxException(__FILE__, __LINE__, s, "invalid Encoded data", (int)i);
      }
      ...
      if (d1 == -1 || d2 == -1) {
        throw URISyntaxException(__FILE__, __LINE__, s, "Invalid Hex Digit in char", (int)i);
      }
...
void URIEncoderDecoder::validateSimple(const std::string &s, const std::string &legal) {
  ...
  for (int i = 0; itr != s.end(); ++i, ++itr) {
    if (Character::isLetterOrDigit(*itr) || legal.find(*itr) != std::string::npos) {
      continue;
    }

    throw URISyntaxException(__FILE__, __LINE__, s, "string contains invalid ASCII chars", (int)i);
  }
}
...
std::string URIEncoderDecoder::decode(const std::string &s) {
  ...
  for (std::size_t i = 0; i < s.length();) {
    ...
    if (c == '%') {
      do {
        if (i + 2 >= s.length()) {
          throw IllegalArgumentException(__FILE__, __LINE__, "String has invalid encoding: %s", s.c_str());
        }
        ...
        if (d1 == -1 || d2 == -1) {
          throw IllegalArgumentException(__FILE__, __LINE__, "URIEncoderDecoder::decode - Bad hex encoding in: ", s.c_str());
        }
