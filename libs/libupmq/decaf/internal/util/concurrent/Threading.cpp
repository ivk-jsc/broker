...
ThreadingLibrary *library = nullptr;
...
void threadExit(ThreadHandle *self, bool destroy = false) {
  ...
  if (self->interruptingThread) {
    ...
    self->interruptingThread = nullptr;
  }
  ...
  PlatformThread::setTlsValue(library->threadKey, nullptr);
  PlatformThread::setTlsValue(library->selfKey, nullptr);
...
void runCallback(void *arg) {
  ...
  try {
    ...
    if (thread->parent->getUncaughtExceptionHandler() != nullptr) {
      thread->parent->getUncaughtExceptionHandler()->uncaughtException(thread->parent, error);
    } else if (thread->parent->getDefaultUncaughtExceptionHandler() != nullptr) {
    ...
    const RuntimeException error(__FILE__, __LINE__, "Uncaught exception bubbled up to Thread::run, Thread Terminating.");
...
void interruptionThread(void *arg) {
  ...
  target->interruptingThread = nullptr;
...
void createThreadInstance(ThreadHandle *thread, long long stackSize, int priority, bool suspended, threadingTask threadMain, void *threadArg) {
  ...
  PlatformThread::createNewThread(&thread->handle, threadEntryMethod, thread, library->priorityMapping[priority], stackSize, &thread->threadId);
}
...
ThreadHandle *initThreadHandle(ThreadHandle *thread) {
  ...
  thread->interruptingThread = nullptr;
  thread->osThread = false;
  thread->handle = PlatformThread::getCurrentThread();
  thread->threadId = 0;
  thread->next = nullptr;
  thread->joiners = nullptr;
  thread->interruptingThread = nullptr;
  thread->monitor = nullptr;
...
void threadExitTlsCleanup(ThreadHandle *thread) {
  for (int index = 0; index < DECAF_MAX_TLS_SLOTS; ++index) {
    if (thread->tls[index] != nullptr) {
      PlatformThread::lockMutex(library->tlsLock);
      void *value = thread->tls[index];
      ThreadLocalImpl *handler = library->tlsSlots[index];
...
void unblockThreads(ThreadHandle *queueHead) {
  ThreadHandle *current = nullptr;
  ThreadHandle *next = nullptr;
...
void dequeueThread(ThreadHandle **queue, ThreadHandle *thread) {
  ...
  if (current == thread) {
    *queue = thread->next;
    ...
    while ((next = current->next) != nullptr && next != thread) {
      current = next;
    }
...
void purgeMonitorsPool(MonitorPool *pool) {
  MonitorHandle *current = nullptr;
  MonitorHandle *next = nullptr;
...
////////////////////////////////////////////////////////////////////////////////
void Threading::lockThreadsLib() { PlatformThread::lockMutex(library->globalLock); }

////////////////////////////////////////////////////////////////////////////////
void Threading::unlockThreadsLib() { PlatformThread::unlockMutex(library->globalLock); }
...
ThreadHandle *Threading::attachToCurrentThread() {
  ...
  thread->name = ::strdup(std::string(std::string("OS-Thread") + Integer::toString(library->osThreadId.getAndIncrement())).c_str());
...
////////////////////////////////////////////////////////////////////////////////
void Threading::yeild() { PlatformThread::yeild(); }
...
bool Threading::join(ThreadHandle *thread, long long mills, int nanos) {
  ...
  if (self->interrupted == true) {
    ...
    JoinCompletionCondition completion(self, nullptr);
    ...
    if (mills > 0 || nanos > 0) {
      self->timerSet = true;
      timedOut = PlatformThread::interruptibleWaitOnCondition(self->condition, self->mutex, mills, nanos, completion);
    } else {
    ...
    if (thread->state >= Thread::RUNNABLE && thread->state != Thread::TERMINATED) {
      ...
      if (mills > 0 || nanos > 0) {
        ...
        PlatformThread::unlockMutex(self->mutex);
        timedOut = PlatformThread::interruptibleWaitOnCondition(self->condition, thread->mutex, mills, nanos, completion);
      } else {
        PlatformThread::unlockMutex(self->mutex);
...
////////////////////////////////////////////////////////////////////////////////
bool Threading::isThreadAlive(ThreadHandle *handle DECAF_UNUSED) { return handle->state >= Thread::RUNNABLE && handle->state != Thread::TERMINATED; }

////////////////////////////////////////////////////////////////////////////////
long long Threading::getThreadId(ThreadHandle *handle) { return handle->threadId; }

////////////////////////////////////////////////////////////////////////////////
int Threading::getThreadPriority(ThreadHandle *handle) { return handle->priority; }
...
////////////////////////////////////////////////////////////////////////////////
const char *Threading::getThreadName(ThreadHandle *handle) { return handle->name; }
...
////////////////////////////////////////////////////////////////////////////////
Thread::State Threading::getThreadState(ThreadHandle *handle) { return (Thread::State)handle->state; }

////////////////////////////////////////////////////////////////////////////////
Thread *Threading::getCurrentThread() { return getCurrentThreadHandle()->parent; }
...
bool Threading::park(Thread *thread, long long mills, int nanos) {
  ...
  if (handle->unparked == true) {
    ...
    if (mills > 0 || nanos > 0) {
      ...
      timedOut = PlatformThread::interruptibleWaitOnCondition(handle->condition, handle->mutex, mills, nanos, completion);
    } else {
...
MonitorHandle *Threading::takeMonitor(bool alreadyLocked) {
  ...
  if (library->monitors->head == nullptr) {
    library->monitors->head = batchAllocateMonitors();
    library->monitors->count = MONITOR_POOL_BLOCK_SIZE;
  }
...
////////////////////////////////////////////////////////////////////////////////
void Threading::notifyWaiter(MonitorHandle *monitor) { doNotifyWaiters(monitor, false); }

////////////////////////////////////////////////////////////////////////////////
void Threading::notifyAllWaiters(MonitorHandle *monitor) { doNotifyWaiters(monitor, true); }
...
int Threading::createThreadLocalSlot(ThreadLocalImpl *threadLocal) {
  ...
  for (index = 0; index < DECAF_MAX_TLS_SLOTS; index++) {
    if (library->tlsSlots[index] == nullptr) {
