...
////////////////////////////////////////////////////////////////////////////////
void PlatformThread::lockMutex(decaf_mutex_t mutex) { ::EnterCriticalSection(mutex); }

////////////////////////////////////////////////////////////////////////////////
bool PlatformThread::tryLockMutex(decaf_mutex_t mutex) { return ::TryEnterCriticalSection(mutex) > 0 ? true : false; }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::unlockMutex(decaf_mutex_t mutex) { ::LeaveCriticalSection(mutex); }
...
void PlatformThread::createRWMutex(decaf_rwmutex_t *mutex) {
  *mutex = new RWLOCK;
  ...
  (*mutex)->readers = 0;
  (*mutex)->readEvent = ::CreateEvent(nullptr, TRUE, FALSE, nullptr);
  if (!((*mutex)->readEvent)) {
    *mutex = nullptr;
    throw RuntimeException(__FILE__, __LINE__, "Failed to initialize OS Event object.");
  }
  (*mutex)->writeMutex = ::CreateMutex(nullptr, FALSE, nullptr);
  if (!((*mutex)->writeMutex)) {
    ::CloseHandle((*mutex)->readEvent);
    *mutex = nullptr;
    throw RuntimeException(__FILE__, __LINE__, "Failed to initialize OS Mutex object.");
  }
}
...
void PlatformThread::readerLockMutex(decaf_rwmutex_t mutex) {
  ...
  if (code == WAIT_FAILED || code == WAIT_TIMEOUT) {
    throw RuntimeException(__FILE__, __LINE__, "Failed to wait for lock on RW object.");
  }
  ...
  if (!::ResetEvent(mutex->readEvent)) {
    throw RuntimeException(__FILE__, __LINE__, "Failed to reset RW Event object.");
  }

  if (!ReleaseMutex(mutex->writeMutex)) {
    throw RuntimeException(__FILE__, __LINE__, "Failed to release RW Mutex object.");
  }
}
  ...
  if (result == ERROR_NOT_OWNER) {
    ...
    if (mutex->readers > 0 && ::InterlockedDecrement(&mutex->readers) == 0) {
      if (!::SetEvent(mutex->readEvent)) {
        throw RuntimeException(__FILE__, __LINE__, "Failed to signal OS event object.");
      }
    }
  } else if (result != 0) {
    throw RuntimeException(__FILE__, __LINE__, "Failed to signal unlock OS Mutex object.");
  }
}
...
void PlatformThread::createCondition(decaf_condition_t *condition) {
  *condition = ::CreateEvent(nullptr, FALSE, FALSE, nullptr);

  if (*condition == INVALID_HANDLE_VALUE) {
    throw RuntimeException(__FILE__, __LINE__, "Failed to initialize OS Condition object.");
  }
}

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::notify(decaf_condition_t condition) { ::SetEvent(condition); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::notifyAll(decaf_condition_t condition) { ::SetEvent(condition); }
...
bool PlatformThread::waitOnCondition(decaf_condition_t condition, decaf_mutex_t mutex, long long mills, int nanos) {
  ...
  ::ResetEvent(condition);
  PlatformThread::unlockMutex(mutex);
  int result = ::WaitForSingleObject(condition, static_cast<DWORD>(mills));
  PlatformThread::lockMutex(mutex);
...
bool PlatformThread::interruptibleWaitOnCondition(
  ...
  do {
    ::ResetEvent(condition);
    PlatformThread::unlockMutex(mutex);
    DWORD timedOut = ::WaitForSingleObject(condition, static_cast<DWORD>(timeOut));
    PlatformThread::lockMutex(mutex);
...
////////////////////////////////////////////////////////////////////////////////
void PlatformThread::destroyCondition(decaf_condition_t condition) { ::CloseHandle(condition); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::initPriorityMapping(int maxPriority, std::vector<int> &mapping) {
  mapping.clear();
  mapping.resize(static_cast<size_t>(maxPriority) + 1);
...
void PlatformThread::createNewThread(
  ...
  *handle = (HANDLE)_beginthreadex(nullptr, (DWORD)0, threadMain, threadArg, 0, &osThreadId);
...
////////////////////////////////////////////////////////////////////////////////
void PlatformThread::detachThread(decaf_thread_t handle) { DECAF_UNUSED_VAR(handle); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::detachOSThread(decaf_thread_t handle) { ::CloseHandle(handle); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::joinThread(decaf_thread_t handle) { ::WaitForSingleObject(handle, INFINITE); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::exitThread() { _endthreadex(0); }

////////////////////////////////////////////////////////////////////////////////
decaf_thread_t PlatformThread::getCurrentThread() { return ::GetCurrentThread(); }

////////////////////////////////////////////////////////////////////////////////
decaf_thread_t PlatformThread::getSafeOSThreadHandle() {
  decaf_thread_t value;

  ::DuplicateHandle(::GetCurrentProcess(), ::GetCurrentThread(), ::GetCurrentProcess(), &value, 0, TRUE, DUPLICATE_SAME_ACCESS);

  return value;
}

////////////////////////////////////////////////////////////////////////////////
long long PlatformThread::getCurrentThreadId() { return ::GetCurrentThreadId(); }
...
////////////////////////////////////////////////////////////////////////////////
void PlatformThread::yeild() { SwitchToThread(); }
...
////////////////////////////////////////////////////////////////////////////////
void PlatformThread::destroyTlsKey(decaf_tls_key tlsKey) { ::TlsFree(tlsKey); }

////////////////////////////////////////////////////////////////////////////////
void *PlatformThread::getTlsValue(decaf_tls_key tlsKey) { return ::TlsGetValue(tlsKey); }

////////////////////////////////////////////////////////////////////////////////
void PlatformThread::setTlsValue(decaf_tls_key tlsKey, void *value) { ::TlsSetValue(tlsKey, value); }
