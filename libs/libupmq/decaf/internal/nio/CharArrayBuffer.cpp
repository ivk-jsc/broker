...
///////////////////////////////////////////////////////////////////////////////
CharArrayBuffer::CharArrayBuffer(int size, bool readOnly) : CharBuffer(size), _array(new ByteArrayAdapter(size)), offset(0), length(size), readOnly(readOnly) {}

///////////////////////////////////////////////////////////////////////////////
CharArrayBuffer::CharArrayBuffer(char *array, int size, int offset, int length, bool readOnly) : CharBuffer(length), _array(), offset(offset), length(length), readOnly(readOnly) {
  try {
    ...
    if (length < 0 || offset + length > size) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "length parameter if out of bounds, %d", length);
    }
...
///////////////////////////////////////////////////////////////////////////////
CharArrayBuffer::CharArrayBuffer(const CharArrayBuffer &other) : CharBuffer(other), _array(other._array), offset(other.offset), length(other.length), readOnly(other.readOnly) {}
...
char *CharArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "CharArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "CharArrayBuffer::array - Buffer is Read-Only");
    }
...
int CharArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "CharArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
CharBuffer &CharArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "CharArrayBuffer::compact() - Buffer is Read Only.");
    }
...
char CharArrayBuffer::get(int index) const {
  try {
    ...
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "CharArrayBuffer::get - Not enough data to fill request.");
    }
...
CharBuffer &CharArrayBuffer::put(int index, char value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "CharArrayBuffer::put(i,i) - Buffer is Read Only.");
    }
    ...
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "CharArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
CharBuffer *CharArrayBuffer::slice() const {
  try {
    return new CharArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
...
CharSequence *CharArrayBuffer::subSequence(int start, int end) const {
  try {
    if (start < 0) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "start index < 0");
    }

    if (end < 0) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "end index < 0");
    }

    if (start > end) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "CharArrayBuffer::subSequence - start > end");
    }

    if (start > this->limit() || end > this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "CharArrayBuffer::subSequence - Sequence exceed limit");
    }
