...
///////////////////////////////////////////////////////////////////////////////
FloatArrayBuffer::FloatArrayBuffer(int size, bool readOnly_) : FloatBuffer(size), _array(), offset(0), length(size), readOnly(readOnly_) {
...
///////////////////////////////////////////////////////////////////////////////
FloatArrayBuffer::FloatArrayBuffer(float *array, int size, int offset_, int length_, bool readOnly_)
    : FloatBuffer(length_), _array(), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
///////////////////////////////////////////////////////////////////////////////
FloatArrayBuffer::FloatArrayBuffer(const Pointer<ByteArrayAdapter> &array, int offset_, int length_, bool readOnly_)
    : FloatBuffer(length_), _array(array), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
float *FloatArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "FloatArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "FloatArrayBuffer::array - Buffer is Read-Only");
    }
...
int FloatArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "FloatArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
FloatBuffer &FloatArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "FloatArrayBuffer::compact() - Buffer is Read Only.");
    }
...
float FloatArrayBuffer::get(int index) const {
  try {
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "FloatArrayBuffer::get - Not enough data to fill request.");
    }
...
FloatBuffer &FloatArrayBuffer::put(int index, float value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "FloatArrayBuffer::put(i,i) - Buffer is Read Only.");
    }

    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "FloatArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
FloatBuffer *FloatArrayBuffer::slice() const {
  try {
    return new FloatArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
