...
///////////////////////////////////////////////////////////////////////////////
LongArrayBuffer::LongArrayBuffer(int size, bool readOnly_) : LongBuffer(size), _array(), offset(0), length(size), readOnly(readOnly_) {
...
///////////////////////////////////////////////////////////////////////////////
LongArrayBuffer::LongArrayBuffer(long long *array, int size, int offset_, int length_, bool readOnly_)
    : LongBuffer(length_), _array(), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
///////////////////////////////////////////////////////////////////////////////
LongArrayBuffer::LongArrayBuffer(const Pointer<ByteArrayAdapter> &array, int offset_, int length_, bool readOnly_)
    : LongBuffer(length_), _array(array), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
long long *LongArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "LongArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "LongArrayBuffer::array - Buffer is Read-Only");
    }
...
int LongArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "LongArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
LongBuffer &LongArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "LongArrayBuffer::compact() - Buffer is Read Only.");
    }
...
long long LongArrayBuffer::get(int index) const {
  try {
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "LongArrayBuffer::get - Not enough data to fill request.");
    }
...
LongBuffer &LongArrayBuffer::put(int index, long long value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "LongArrayBuffer::put(i,i) - Buffer is Read Only.");
    }

    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "LongArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
LongBuffer *LongArrayBuffer::slice() const {
  try {
    return new LongArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
