...
///////////////////////////////////////////////////////////////////////////////
ShortArrayBuffer::ShortArrayBuffer(int size, bool readOnly_) : ShortBuffer(size), _array(), offset(0), length(size), readOnly(readOnly_) {
...
///////////////////////////////////////////////////////////////////////////////
ShortArrayBuffer::ShortArrayBuffer(short *array, int size, int offset_, int length_, bool readOnly_)
    : ShortBuffer(length_), _array(), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
///////////////////////////////////////////////////////////////////////////////
ShortArrayBuffer::ShortArrayBuffer(const Pointer<ByteArrayAdapter> &array, int offset_, int length_, bool readOnly_)
    : ShortBuffer(length_), _array(array), offset(offset_), length(length_), readOnly(readOnly_) {
  try {
...
short *ShortArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "ShortArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "ShortArrayBuffer::array - Buffer is Read-Only");
    }
...
int ShortArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "ShortArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
ShortBuffer &ShortArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "ShortArrayBuffer::compact() - Buffer is Read Only.");
    }
...
short ShortArrayBuffer::get(int index) const {
  try {
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "ShortArrayBuffer::get - Not enough data to fill request.");
    }
...
ShortBuffer &ShortArrayBuffer::put(int index, short value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "ShortArrayBuffer::put(i,i) - Buffer is Read Only.");
    }

    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "ShortArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
ShortBuffer *ShortArrayBuffer::slice() const {
  try {
    return new ShortArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
