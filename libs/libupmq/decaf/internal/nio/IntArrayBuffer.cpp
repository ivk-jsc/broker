...
int *IntArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "IntArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "IntArrayBuffer::array - Buffer is Read-Only");
    }
...
int IntArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "IntArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
IntBuffer &IntArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "IntArrayBuffer::compact() - Buffer is Read Only.");
    }
...
int IntArrayBuffer::get(int index) const {
  try {
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "IntArrayBuffer::get - Not enough data to fill request.");
    }
...
IntBuffer &IntArrayBuffer::put(int index, int value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "IntArrayBuffer::put(i,i) - Buffer is Read Only.");
    }

    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "IntArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
IntBuffer *IntArrayBuffer::slice() const {
  try {
    return new IntArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
