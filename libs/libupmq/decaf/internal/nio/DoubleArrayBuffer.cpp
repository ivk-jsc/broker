...
double *DoubleArrayBuffer::array() {
  try {
    if (!this->hasArray()) {
      throw UnsupportedOperationException(__FILE__, __LINE__, "DoubleArrayBuffer::arrayOffset() - This Buffer has no backing array.");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "DoubleArrayBuffer::array - Buffer is Read-Only");
    }
...
int DoubleArrayBuffer::arrayOffset() {
  try {
    ...
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "DoubleArrayBuffer::arrayOffset() - Buffer is Read Only.");
    }
...
DoubleBuffer &DoubleArrayBuffer::compact() {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "DoubleArrayBuffer::compact() - Buffer is Read Only.");
    }
...
double DoubleArrayBuffer::get(int index) const {
  try {
    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "DoubleArrayBuffer::get - Not enough data to fill request.");
    }
...
DoubleBuffer &DoubleArrayBuffer::put(int index, double value) {
  try {
    if (this->isReadOnly()) {
      throw decaf::nio::ReadOnlyBufferException(__FILE__, __LINE__, "DoubleArrayBuffer::put(i,i) - Buffer is Read Only.");
    }

    if (index >= this->limit()) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "DoubleArrayBuffer::put(i,i) - Not enough data to fill request.");
    }
...
DoubleBuffer *DoubleArrayBuffer::slice() const {
  try {
    return new DoubleArrayBuffer(this->_array, this->offset + this->position(), this->remaining(), this->isReadOnly());
  }
  DECAF_CATCH_RETHROW(Exception)
  DECAF_CATCHALL_THROW(Exception)
}
