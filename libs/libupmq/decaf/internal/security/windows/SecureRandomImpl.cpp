...
SecureRandomImpl::SecureRandomImpl() {
  ...
  try {
    HCRYPTPROV provider;
    int result = CryptAcquireContext(&provider, nullptr, nullptr, PROV_DSS, CRYPT_VERIFYCONTEXT);
...
void SecureRandomImpl::providerNextBytes(unsigned char *bytes, int numBytes) {
  ...
  if (this->config->random.get() == nullptr) {
    HCRYPTPROV provider;

    int result;

    result = CryptAcquireContext(&provider, nullptr, nullptr, PROV_DSS, CRYPT_VERIFYCONTEXT);

    if (result == 0) {
      throw RuntimeException(__FILE__, __LINE__, "Failed to acquire the system cryptographic provider.");
    }

    result = CryptGenRandom(provider, numBytes, bytes);

    if (result == 0) {
      throw RuntimeException(__FILE__, __LINE__, "Failed to get random bytes from the cryptographic provider.");
    }
