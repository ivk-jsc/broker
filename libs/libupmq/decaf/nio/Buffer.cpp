...
Buffer &Buffer::position(int newPosition) {
  if (newPosition < 0 || newPosition > this->_limit) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Buffer::position - New Position is greater than set limit");
  }
...
Buffer &Buffer::limit(int newLimit) {
  if (newLimit < 0 || newLimit > this->capacity()) {
    throw IllegalArgumentException(__FILE__, __LINE__, "Buffer::limit - new limit is larger than the capacity.");
  }
...
Buffer &Buffer::reset() {
  if (!this->_markSet) {
    throw InvalidMarkException(__FILE__, __LINE__, "Buffer::reset - Buffer has not been Marked.");
  }
