...
IntBuffer *IntBuffer::wrap(int *buffer, int size, int offset, int length) {
  try {
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "IntBuffer::wrap - Passed Buffer is Null.");
    }
...
IntBuffer *IntBuffer::wrap(std::vector<int> &buffer) {
  try {
    if (buffer.empty()) {
      throw NullPointerException(__FILE__, __LINE__, "IntBuffer::wrap - Passed Buffer is Empty.");
    }
...
IntBuffer &IntBuffer::get(int *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "IntBuffer::get - Passed Buffer is Null");
    }
    ...
    if (length > remaining()) {
      throw BufferUnderflowException(__FILE__, __LINE__, "IntBuffer::get - Not enough data to fill length = %d", length);
    }
...
IntBuffer &IntBuffer::put(IntBuffer &src) {
  try {
    if (this == &src) {
      throw IllegalArgumentException(__FILE__, __LINE__, "IntBuffer::put - Can't put Self");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "IntBuffer::put - This buffer is Read Only.");
    }

    if (src.remaining() > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "IntBuffer::put - Not enough space remaining to put src.");
    }
...
IntBuffer &IntBuffer::put(const int *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "IntBuffer::put - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "IntBuffer::put - Not Enough space to store requested Data.");
    }
...
////////////////////////////////////////////////////////////////////////////////
bool IntBuffer::operator==(const IntBuffer &value) const { return this->equals(value); }

////////////////////////////////////////////////////////////////////////////////
bool IntBuffer::operator<(const IntBuffer &value) const { return this->compareTo(value) < 0 ? true : false; }
