...
CharBuffer *CharBuffer::allocate(int capacity) {
  try {
    if (capacity < 0) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "Capacity given was negative.");
    }
...
CharBuffer *CharBuffer::wrap(char *buffer, int size, int offset, int length) {
  try {
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "CharBuffer::wrap - Passed Buffer is Null.");
    }
...
CharBuffer *CharBuffer::wrap(std::vector<char> &buffer) {
  try {
    if (buffer.empty()) {
      throw NullPointerException(__FILE__, __LINE__, "CharBuffer::wrap - Passed Buffer is Empty.");
    }
...
CharBuffer &CharBuffer::get(char *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "CharBuffer::get - Passed Buffer is Null");
    }
    ...
    if (length > remaining()) {
      throw BufferUnderflowException(__FILE__, __LINE__, "CharBuffer::get - Not enough data to fill length = %d", length);
    }
...
CharBuffer &CharBuffer::put(CharBuffer &src) {
  try {
    if (this == &src) {
      throw IllegalArgumentException(__FILE__, __LINE__, "CharBuffer::put - Can't put Self");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "CharBuffer::put - This buffer is Read Only.");
    }

    if (src.remaining() > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "CharBuffer::put - Not enough space remaining to put src.");
    }
...
CharBuffer &CharBuffer::put(const char *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "CharBuffer::put - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "CharBuffer::put - Not Enough space to store requested Data.");
    }
...
CharBuffer &CharBuffer::put(std::string &src, int start, int end) {
  try {
    if ((start > end) || ((int)src.size() < (end - start))) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "CharBuffer::put - invalid start and end pos; start = %d, end = %d", start, end);
    }
...
int CharBuffer::read(CharBuffer *target) {
  try {
    if (target == this) {
      throw IllegalArgumentException(__FILE__, __LINE__, "CharBuffer::read - Cannot read to self");
    }

    if (target == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "CharBuffer::read - Null CharBuffer Passed");
    }
...
////////////////////////////////////////////////////////////////////////////////
bool CharBuffer::operator==(const CharBuffer &value) const { return this->equals(value); }

////////////////////////////////////////////////////////////////////////////////
bool CharBuffer::operator<(const CharBuffer &value) const { return this->compareTo(value) < 0 ? true : false; }
