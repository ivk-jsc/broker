...
ByteBuffer *ByteBuffer::wrap(unsigned char *buffer, int size, int offset, int length) {
  try {
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "ByteBuffer::wrap - Passed Buffer is Null.");
    }

    if (size < 0 || offset < 0 || length < 0 || offset + length > size) {
      throw IndexOutOfBoundsException(__FILE__, __LINE__, "Invalid array access parameters passed.");
    }
...
ByteBuffer *ByteBuffer::wrap(std::vector<unsigned char> &buffer) {
  try {
    if (buffer.empty()) {
      throw NullPointerException(__FILE__, __LINE__, "ByteBuffer::wrap - Passed Buffer is Empty.");
    }
...
ByteBuffer &ByteBuffer::get(unsigned char *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "ByteBuffer::get - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferUnderflowException(__FILE__, __LINE__, "ByteBuffer::get - Not Enough Data to Fill Request.");
    }
...
ByteBuffer &ByteBuffer::put(ByteBuffer &src) {
  try {
    if (this == &src) {
      throw IllegalArgumentException(__FILE__, __LINE__, "ByteBuffer::put - Can't put Self");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "ByteBuffer::put - This buffer is Read Only.");
    }

    if (src.remaining() > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "ByteBuffer::put - Not enough space remaining to put src.");
    }
...
ByteBuffer &ByteBuffer::put(const unsigned char *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "ByteBuffer::put - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "ByteBuffer::put - Not Enough space to store requested Data.");
    }
...
int ByteBuffer::compareTo(const ByteBuffer &value) const {
  try {
    int compareRemaining = (remaining() < value.remaining()) ? remaining() : value.remaining();
...
////////////////////////////////////////////////////////////////////////////////
bool ByteBuffer::operator==(const ByteBuffer &value) const { return this->equals(value); }

////////////////////////////////////////////////////////////////////////////////
bool ByteBuffer::operator<(const ByteBuffer &value) const { return this->compareTo(value) == -1; }
