...
DoubleBuffer *DoubleBuffer::wrap(double *buffer, int size, int offset, int length) {
  try {
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "DoubleBuffer::wrap - Passed Buffer is Null.");
    }
...
DoubleBuffer *DoubleBuffer::wrap(std::vector<double> &buffer) {
  try {
    if (buffer.empty()) {
      throw NullPointerException(__FILE__, __LINE__, "DoubleBuffer::wrap - Passed Buffer is Empty.");
    }
...
DoubleBuffer &DoubleBuffer::get(double *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "DoubleBuffer::get - Passed Buffer is Null");
    }
    ...
    if (length > remaining()) {
      throw BufferUnderflowException(__FILE__, __LINE__, "DoubleBuffer::get - Not enough data to fill length = %d", length);
    }
...
DoubleBuffer &DoubleBuffer::put(DoubleBuffer &src) {
  try {
    if (this == &src) {
      throw IllegalArgumentException(__FILE__, __LINE__, "DoubleBuffer::put - Can't put Self");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "DoubleBuffer::put - This buffer is Read Only.");
    }

    if (src.remaining() > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "DoubleBuffer::put - Not enough space remaining to put src.");
    }
...
DoubleBuffer &DoubleBuffer::put(const double *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "DoubleBuffer::put - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "DoubleBuffer::put - Not Enough space to store requested Data.");
    }
...
////////////////////////////////////////////////////////////////////////////////
bool DoubleBuffer::operator==(const DoubleBuffer &value) const { return this->equals(value); }

////////////////////////////////////////////////////////////////////////////////
bool DoubleBuffer::operator<(const DoubleBuffer &value) const { return this->compareTo(value) < 0 ? true : false; }
