...
FloatBuffer *FloatBuffer::wrap(float *buffer, int size, int offset, int length) {
  try {
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "FloatBuffer::wrap - Passed Buffer is Null.");
    }
...
FloatBuffer *FloatBuffer::wrap(std::vector<float> &buffer) {
  try {
    if (buffer.empty()) {
      throw NullPointerException(__FILE__, __LINE__, "FloatBuffer::wrap - Passed Buffer is Empty.");
    }
...
FloatBuffer &FloatBuffer::get(float *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "FloatBuffer::get - Passed Buffer is Null");
    }
    ...
    if (length > remaining()) {
      throw BufferUnderflowException(__FILE__, __LINE__, "FloatBuffer::get - Not enough data to fill length = %d", length);
    }
...
FloatBuffer &FloatBuffer::put(FloatBuffer &src) {
  try {
    if (this == &src) {
      throw IllegalArgumentException(__FILE__, __LINE__, "FloatBuffer::put - Can't put Self");
    }

    if (this->isReadOnly()) {
      throw ReadOnlyBufferException(__FILE__, __LINE__, "FloatBuffer::put - This buffer is Read Only.");
    }

    if (src.remaining() > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "FloatBuffer::put - Not enough space remaining to put src.");
    }
...
FloatBuffer &FloatBuffer::put(const float *buffer, int size, int offset, int length) {
  try {
    ...
    if (buffer == nullptr) {
      throw NullPointerException(__FILE__, __LINE__, "FloatBuffer::put - Passed Buffer is Null.");
    }
    ...
    if (length > this->remaining()) {
      throw BufferOverflowException(__FILE__, __LINE__, "FloatBuffer::put - Not Enough space to store requested Data.");
    }
...
////////////////////////////////////////////////////////////////////////////////
bool FloatBuffer::operator==(const FloatBuffer &value) const { return this->equals(value); }

////////////////////////////////////////////////////////////////////////////////
bool FloatBuffer::operator<(const FloatBuffer &value) const { return this->compareTo(value) < 0 ? true : false; }
