...
class CMS_API BytesMessage : public Message {
 public:
  ...
  /**
   * sets the bytes given to the message body from stream.
   *
   * @param input
   *      Bytes Stream to copy
   *
   * @throws CMSException - If an internal error occurs.
   * @throws MessageNotWriteableException - if in Read Only Mode.
   */
  virtual void setBodyBytesFromStream(std::istream &input) = 0;

  /**
   * Gets the bytes that are contained in this message and returns them in a stream
   * that becomes the property of the caller.  This is a copy of the
   * data contained in this message, changing the value contained in this stream has
   * no effect on the data contained in this message.
   *
   *
   * @throws CMSException - If an internal error occurs.
   * @throws MessageNotReadableException - If the message is in Write Only Mode.
   */
  virtual void getBodyBytesToStream(std::ostream &output) const = 0;
