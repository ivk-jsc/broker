...
class CMS_API MessageProducer : public Closeable {
 public:
   ...
   *         MessageProducer that did not specify a destination at creation time.
   */
  // virtual void send(Message *message, int deliveryMode, int priority, long long timeToLive, AsyncCallback
  // *onComplete) = 0;
   ...
   *         MessageProducer that did not specify a destination at creation time.
   */
  // virtual void send(const Destination *destination, Message *message, int deliveryMode, int priority, long long
  // timeToLive, AsyncCallback *onComplete) = 0;
