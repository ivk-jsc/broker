...
public class TcpTransport extends TransportThreadSupport implements Transport, Service, Runnable {
	...
	/**
	 * Prevents setting both the Differentiated Services and Type of Service transport options at the same time, since
	 * they share the same spot in the TCP/IP packet headers.
	 */
	...
	/**
	 * Specifies the port that will be used by the JMX server to manage the TransportLoggers. This should only be set in
	 * an URI by a client (producer or consumer) since a broker will already create a JMX server. It is useful for
	 * people who test a broker and clients in the same machine and want to control both via JMX; a different port will
	 * be needed.
	 */
	...
	public String toString() {
		return "" + (socket.isConnected() ? "tcp://" + socket.getInetAddress() + ":" + socket.getPort() + "@" + socket.getLocalPort() : (localLocation != null ? localLocation : remoteLocation));
	}
	...
	/**
	 * Sets whether 'localhost' or the actual local host name should be used to make local connections. On some
	 * operating systems such as Macs its not possible to connect as the local host name so localhost is better.
	 */
	 ...
	 *             IllegalArgumentException if setting the options on the socket failed.
	 */
	...
	/**
	 * @param sock
	 *            The socket on which to set the Traffic Class.
	 * @return Whether or not the Traffic Class was set on the given socket.
	 * @throws SocketException
	 *             if the system does not support setting the Traffic Class.
	 * @throws IllegalArgumentException
	 *             if both the Differentiated Services and Type of Services transport options have been set on the same
	 *             connection.
	 */
	private boolean setTrafficClass(Socket sock) throws SocketException, IllegalArgumentException {
		...
		if (this.trafficClass != resultTrafficClass) {
			...
			if ((this.trafficClass >> 2) == (resultTrafficClass >> 2) && (this.trafficClass & 3) != (resultTrafficClass & 3)) {
				LOG.warn("Attempted to set the Traffic Class to " + this.trafficClass + " but the result Traffic Class was " + resultTrafficClass + ". Please check that your system "
