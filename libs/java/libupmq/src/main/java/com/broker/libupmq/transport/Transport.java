...
public interface Transport extends Service {
	...
	/**
	 * An asynchronous request response where the Receipt will be returned in the future. If responseCallback is not
	 * null, then it will be called when the response has been completed.
	 *
	 * @param command
	...
	/**
	 * Returns a counter which gets incremented as data is read from the transport. It should only be used to determine
	 * if there is progress being made in reading the next command from the transport. The value may wrap into the
	 * negative numbers.
	 *
