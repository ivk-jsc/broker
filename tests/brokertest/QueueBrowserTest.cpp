...
TEST_F(QueueBrowserTest, testReceiveBrowseReceive) {
  ...
  EXPECT_NO_THROW(inbound.reset(dynamic_cast<cms::TextMessage *>(consumer->receive(cmsProvider->minTimeout))));
...
TEST_F(QueueBrowserTest, testBrowseReceive) {
  ...
  inbound.reset(dynamic_cast<cms::TextMessage *>(consumer->receive(cmsProvider->minTimeout)));
  EXPECT_TRUE(inbound.get() != nullptr);
  EXPECT_EQ(message1->getText(), inbound->getText());
...
TEST_F(QueueBrowserTest, testQueueBrowserWith2Consumers) {
  ...
  for (int i = 0; i < numMessages; i++) {
    std::unique_ptr<cms::Message> m1(consumer->receive(cmsProvider->minTimeout));
    sstr.str("");
